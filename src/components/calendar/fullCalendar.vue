            ...
            <div class="events-day" v-for="day in week" track-by="$index" :class="{'today' : day.isToday,
              ...
              <div class="event-box">
                <p v-if="day.events.length > eventLimit" class="more-link" @click.stop="selectThisDay(day, $event)">
                  <strong>结算价：</strong>{{day.events[day.events.length -1].settlement}} <br>
                  <strong>分销价：</strong>{{day.events[day.events.length -1].distribution}}<br>
                  <strong>库存：</strong>{{day.events[day.events.length -1].inventoryPooling}}<br>
                </p>
              </div>
            </div>
          </div>
